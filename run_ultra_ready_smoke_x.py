...
ENV = os.environ.copy()
TRAINER_LOG = ROOT / 'smoke_logs' / 'ultra_ready_x_trainer.log'
ENV_LOG = ROOT / 'smoke_logs' / 'ultra_ready_x_env.log'
...
def kill_stale_x_processes():
    subprocess.run(
        ['bash', '-lc', 'pkill -f "min_x_strategy_tinker.py serve" || true; pkill -f "launch_training.py --config configs/min_x_strategy" || true'],
        cwd=ROOT,
        env=ENV,
        capture_output=True,
        text=True,
