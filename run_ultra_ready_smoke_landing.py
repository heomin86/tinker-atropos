...
ENV = os.environ.copy()
TRAINER_LOG = ROOT / 'smoke_logs' / 'ultra_ready_landing_trainer.log'
ENV_LOG = ROOT / 'smoke_logs' / 'ultra_ready_landing_env.log'
CFG = 'configs/min_landing_cro_ultra_smoke.yaml'
ENV_PY = 'tinker_atropos/environments/min_landing_cro_tinker.py'
...
def kill_stale_landing_processes():
    subprocess.run(
        ['bash', '-lc', 'pkill -f "min_landing_cro_tinker.py serve" || true; pkill -f "launch_training.py --config configs/min_landing_cro" || true'],
        cwd=ROOT,
        env=ENV,
        capture_output=True,
        text=True,
