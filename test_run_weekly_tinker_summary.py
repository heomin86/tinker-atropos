...
def test_run_weekly_tinker_summary_cli_prints_markdown_and_json(tmp_path):
    ...
    proc = subprocess.run(
        ['python', 'ops/run_weekly_tinker_summary.py', '--root', str(tmp_path)],
        cwd=root,
        capture_output=True,
        text=True,
    )
    ...
    assert 'Weekly Tinker Research Summary' in proc.stdout
