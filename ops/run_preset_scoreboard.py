...
from ops.preset_scoreboard import build_comment_body, collect_preset_scoreboard
from ops.sync_result_to_paperclip import build_issue_update_payload, patch_issue



def main() -> None:
    parser = argparse.ArgumentParser(description='preset 별 full funnel 점수와 최고 headline 을 집계한다.')
    ...
    summary = collect_preset_scoreboard(Path(args.root))
    comment = build_comment_body(summary)

    if args.sync and args.issue_id:
        status = 'done' if summary['preset_count'] > 0 else 'todo'
