...
from ops.full_funnel_status import build_comment_body, collect_full_funnel_status
from ops.sync_result_to_paperclip import build_issue_update_payload, patch_issue



def main() -> None:
    parser = argparse.ArgumentParser(description='최근 full funnel 실행 산출물과 요약 상태를 점검한다.')
    ...
    summary = collect_full_funnel_status(Path(args.root))
    comment = build_comment_body(summary)

    if args.sync and args.issue_id:
        status = 'done' if summary['alert_count'] == 0 and summary['run_count'] > 0 else 'todo'
