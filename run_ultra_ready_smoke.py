...
ENV = os.environ.copy()
TRAINER_LOG = ROOT / 'smoke_logs' / 'ultra_ready_trainer.log'
ENV_LOG = ROOT / 'smoke_logs' / 'ultra_ready_env.log'
...
def kill_stale_business_processes():
    subprocess.run(
        ['bash', '-lc', 'pkill -f "min_business_strategy_tinker.py serve" || true; pkill -f "launch_training.py --config configs/min_business_strategy" || true'],
        cwd=ROOT,
        env=ENV,
        capture_output=True,
        text=True,
