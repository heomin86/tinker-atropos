...
ENV = os.environ.copy()
TRAINER_LOG = ROOT / 'smoke_logs' / 'ultra_ready_retention_trainer.log'
ENV_LOG = ROOT / 'smoke_logs' / 'ultra_ready_retention_env.log'
CFG = 'configs/min_membership_retention_ultra_smoke.yaml'
ENV_PY = 'tinker_atropos/environments/min_membership_retention_tinker.py'
...
def kill_stale_retention_processes():
    subprocess.run(
        ['bash', '-lc', 'pkill -f "min_membership_retention_tinker.py serve" || true; pkill -f "launch_training.py --config configs/min_membership_retention" || true'],
        cwd=ROOT,
        env=ENV,
        capture_output=True,
        text=True,
...
kill_stale_retention_processes()
time.sleep(2)
kill_port(8001)
